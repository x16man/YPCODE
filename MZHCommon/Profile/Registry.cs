...
namespace MZHCommon.Profile
{
	...
	public class Registry : Profile
	{
		...
		/// <exception cref="InvalidOperationException">Application.CompanyName or Application.ProductName are empty.</exception>
		/// <remarks>
		...
		protected RegistryKey GetSubKey(string section, bool create)		
		{
			VerifyName();
			...
			return m_rootKey.OpenSubKey(keyName, true);
		}
		...
		public override void SetValue(string section, string entry, object value)
		{
			...
			using (RegistryKey subKey = GetSubKey(section, true))
		...
		public override object GetValue(string section, string entry)
		{
			...
			using (RegistryKey subKey = GetSubKey(section, false))

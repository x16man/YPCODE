...
namespace ZedGraph
{
	...
	#region Class Fields
		...
		/// </summary>
		/// <remarks>
		/// note that, if you are going to Serialize ZedGraph data, then any type that you store in <see cref="Tag"/> must be a serializable type (or
	...
	#region Defaults
		...
		public struct Default
		{
			...
			/// </summary>
			public static string TitleFontFamily = "宋体";
			/// <summary>
			/// 坐标轴标题字体大小。
			...
			/// </summary>
			public static FillType TitleFillType = FillType.Solid;
	...
	#region Scale Properties
		...
		{
			get { return _cross; }
			set { _cross = value; CrossAuto = false; }
		}
	...
	#region Tic Properties
	    ...
	    /// </summary>
	    public MajorTic MajorTic { get; internal set; }
	    ...
	    /// </summary>
	    public MinorTic MinorTic { get; internal set; }
	...
	#region Grid Properties
	    ...
	    /// of the major grid.
	    /// </summary>
	    public MajorGrid MajorGrid { get; internal set; }

	    /// <summary>
	    /// Gets a reference to the <see cref="MinorGrid" /> class that contains the properties
	    /// of the minor grid.
	    /// </summary>
	    public MinorGrid MinorGrid { get; internal set; }
	...
	#region Label Properties
        ...
        /// <summary>
        /// 多重坐标之间间隙的大小。 (见 <see cref="GraphPane.YAxisList"/> 和
...
//		abstract internal float GetMinPix( GraphPane pane );
		...
		internal float CalcCrossFraction( GraphPane pane )
		{
			...
			if ( ( ( this is XAxis || this is YAxis ) && Scale.IsLabelsInside == crossAxis.Scale.IsReverse ) ||
				 ( ( this is X2Axis || this is Y2Axis ) && Scale.IsLabelsInside != crossAxis.Scale.IsReverse ) )
		...
		private float CalcTotalShift( GraphPane pane, float scaleFactor, float shiftPos )
		{
			if ( !IsPrimary( pane ) )
			{
				...
				else
				{
					...
					float ticSize = MajorTic.ScaledTic( scaleFactor );
		...
		public float CalcSpace( Graphics g, GraphPane pane, float scaleFactor, out float fixedSpace )
		{
			...
			float axisGap = AxisGap * scaleFactor;
			float scaledLabelGap = Scale.LabelGap * charHeight;
			...
			if ( IsVisible )
			{
				bool hasTic = this.MajorTic.IsOutside || this.MajorTic.IsCrossOutside ||
									this.MinorTic.IsOutside || this.MinorTic.IsCrossOutside;
				...
				if ( !IsPrimary( pane ) )
				{
					...
					if ( this.MajorTic.IsInside || this.MajorTic.IsCrossInside ||
							this.MinorTic.IsInside || this.MinorTic.IsCrossInside )
						_tmpSpace += ticSize;
				}
			...
			if ( this.IsPrimary( pane ) && ( (
					( this is YAxis && (
						( !pane.XAxis.Scale.IsSkipFirstLabel && !pane.XAxis.Scale.IsReverse ) ||
						( !pane.XAxis.Scale.IsSkipLastLabel && pane.XAxis.Scale.IsReverse ) ) ) ||
					( this is Y2Axis && (
						( !pane.XAxis.Scale.IsSkipFirstLabel && pane.XAxis.Scale.IsReverse ) ||
						( !pane.XAxis.Scale.IsSkipLastLabel && !pane.XAxis.Scale.IsReverse ) ) ) ) &&
			...
			_tmpSpace = Math.Max( _tmpSpace, MinSpace * (float)scaleFactor );

			fixedSpace = Math.Max( fixedSpace, MinSpace * (float)scaleFactor );
				...
				float left, float right )
		{
			...
			if ( IsVisible && MajorGrid._isZeroLine &&
					Scale._min < 0.0 && Scale._max > 0.0 )
			{
				float zeroPix = Scale.Transform( 0.0 );

				using ( Pen zeroPen = new Pen( Color,
						pane.ScaledPenWidth( MajorGrid._penWidth, scaleFactor ) ) )
				{
								...
								float scaleFactor, float topPix )
		{
			if ( ( this.MinorTic.IsOutside || this.MinorTic.IsOpposite || this.MinorTic.IsInside ||
					this.MinorTic.IsCrossOutside || this.MinorTic.IsCrossInside || MinorGrid._isVisible )
					&& IsVisible )
			{
				...
				if ( Scale.IsLog || tMinor < tMajor )
				{
					...
					using ( Pen	pen = new Pen( MinorTic.Color,
										pane.ScaledPenWidth( MinorTic.PenWidth, scaleFactor ) ) )
					using ( Pen minorGridPen = MinorGrid.GetPen( pane, scaleFactor ) )
					{
		...
		public void DrawTitle( Graphics g, GraphPane pane, float shiftPos, float scaleFactor )
		{
			...
			{
				bool hasTic = ( Scale.IsLabelsInside ?
						( this.MajorTic.IsInside || this.MajorTic.IsCrossInside ||
								this.MinorTic.IsInside || this.MinorTic.IsCrossInside ) :
						( this.MajorTic.IsOutside || this.MajorTic.IsCrossOutside || this.MinorTic.IsOutside || this.MinorTic.IsCrossOutside ) );
				...
				float scaledLabelGap = Scale._fontSpec.GetHeight( scaleFactor ) * Scale.LabelGap;
				float scaledTitleGap = Title.GetScaledGap( scaleFactor );
				...
				float y = ( Scale.IsVisible ? Scale.GetScaleMaxSpace( g, pane, scaleFactor, true ).Height
							+ scaledLabelGap : 0 );
		...
		private string MakeTitle()
		{
			...
			if ( Scale._mag != 0 && !Title._isOmitMag && !Scale.IsLog )

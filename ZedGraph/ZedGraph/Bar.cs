...
namespace ZedGraph
{
   ...
   public class Bar : ICloneable, ISerializable
   {
	...
	#region Constructors
        ...
        public Bar(Color color,Color negativeColor)
        {
            _border = new Border(Default.IsBorderVisible, Default.BorderColor, Default.BorderWidth);
            _fill = new Fill(color.IsEmpty ? Default.FillColor : color,Default.FillBrush, Default.FillType);
            _negativeFill = new Fill(negativeColor.IsEmpty?Default.FillColor:negativeColor,Default.FillBrush,Default.FillType);
        }
		/// <summary>
	...
	#region Rendering Methods
							...
							bool fullFrame, bool isSelected, PointPair dataValue )
		{
			...
			{
                if(_negativeFill != null)//如果对于负值进行特殊处理。
                {
                    if (dataValue.Y >= 0)
                        _fill.Draw(g, rect, dataValue);
                    else
                    {
                        _negativeFill.Draw(g, rect, dataValue);
                    }
                }
                else
                {

...
namespace ZedGraph
{
	...
	#region Events
        ...
        /// <summary>
        /// This event provides for a notification when the mouse is clicked on an object 
        /// within an <see cref="GraphPane"/> of the <see cref="MasterPane"/> associated
        /// with this <see cref="ZedGraphControl"/>. This event will use the
        /// <see cref="ZedGraph.MasterPane.FindNearestPaneObject"/> method to determine whick object
        /// was clicked. The boolean value that return from this handler determines whether
        /// or not the <see cref="ZedMouseEventHandler"/>). Return true if you have handled the 
        /// DoubleClick event entirely,and you do not
        /// want the<see cref="ZedGraphControl"/> to do any further action.
        /// Return false if ZedGraph should go ahead and process the 
        /// MouseClick event.
        /// </summary>
        [Bindable(true), Category("Events"),
         Description("Subscribe to be notified when the left mouse button is double-clicked")]
        public event ZedMouseEventHandler MouseClickEvent;
        /// <summary>
        /// Subscribe to this event to provide notification of Double Clicks on graph
        /// objects
        /// </summary>
        ...
        [Bindable(true), Category("Events"),
         Description("Subscribe to be notified when the left mouse button is double-clicked")]
        public event ZedMouseEventHandler MouseDoubleClickEvent;
        ...
        /// <summary>
        /// A delegate that allows notification of clicks on ZedGraph's title.
        /// </summary>
        /// <param name="Sender">The source <see cref="ZedGraphControl"/> object</param>
        /// <param name="pane">The source <see cref="GraphPane" /> in which the click
        /// occurred.
        /// </param>
        public delegate void TitleHandler(ZedGraphControl Sender, GraphPane pane);
        public event TitleHandler TitleClickEvent;
        public event TitleHandler TitleDoubleClickEvent;
        //TODO: Add TitleHandler
        /// <summary>
        /// A delegate that allows notification of clicks on ZedGraph's legend.
        /// </summary>
        /// <param name="Sender"></param>
        /// <param name="pane"></param>
        /// <param name="curve"></param>
        public delegate void LegendHandler(ZedGraphControl Sender, GraphPane pane, CurveItem curve);
        public event LegendHandler LegendDoubleClickEvent;
        public event LegendHandler LegendClickEvent;
        //TODO: Add LegendHandler
        /// <summary>
        /// A delegate that allows notification of clicks on ZedGraph's Curve's point.
        /// </summary>
        /// <param name="Sender"></param>
        /// <param name="pane"></param>
        /// <param name="curve"></param>
        /// <param name="iPt"></param>
        public delegate void PointHandler(ZedGraphControl Sender, GraphPane pane, CurveItem curve, int iPt);
        public event PointHandler PointClickEvent;
        public event PointHandler PointDoubleClickEvent;
	...
	#region Mouse Events
		...
		protected void ZedGraphControl_MouseDown( object sender, MouseEventArgs e )
		{
            ...
            {
                for (int i = _floatingBlocks.Count - 1; i >= 0; i--)
                {
                    var block = _floatingBlocks[i];
                    if (block.HitTest(e.Location))
                    {
                        block.StartDrag(e.Location);
                        break;
                    }
                }
            }
		...
		protected void ZedGraphControl_MouseUp( object sender, MouseEventArgs e )
		{
            ...
            if (e.Button == MouseButtons.Left)
            {
                for (int i = _floatingBlocks.Count - 1; i >= 0; i--)
                {
                    ...
                    {
                        block.StopDrag();
                        _toolTip_W.Show(String.Format("X:{0},Y:{1}", block.RectF.X, block.RectF.Y), this, Point.Round(block.RectF.Location), 2000);
                        break;
                    }
                }
            }
		...
		protected string MakeValueLabel( Axis axis, double val, int iPt, bool isOverrideOrdinal )
		{
			if ( axis != null )
			{
				...
				else if ( axis.Scale.IsText && axis.Scale.TextLabels != null )
				{
					int i = iPt;
		...
		protected void ZedGraphControl_MouseMove( object sender, MouseEventArgs e )
		{
			if ( _masterPane != null )
			{
                ...
                SetCursor(mousePt); //Commented By Zhanghao 2009-06-08.
                ...
                {
                    this._graphPane = this._masterPane.FindChartRect(mousePt);
                    ...
                    if (this._graphPane == null || this._graphPane.CurveList.IsPieOnly)
                    {
                        if (this._hGuideLine.IsVisible || this._vGuideLine.IsVisible)
                        {
                            this._hGuideLine.IsVisible = this._vGuideLine.IsVisible = false;
                            this._xValueText.IsVisible = this._yValueText.IsVisible = false;
                            isNeedRefresh = true;
                        }
                    }
                    else
                    {
                        this._yValueText.IsVisible = this._vGuideLine.IsVisible = this.IsShowVerticalGuideLine;
                        this._xValueText.IsVisible = this._hGuideLine.IsVisible = this.IsShowHorizontalGuideLine;
                        this.GuidePoint = mousePt;
                        isNeedRefresh = true;
                    }
                }
                if (isNeedRefresh) Invalidate();
			}
		}
        ...
        protected void ZedGraphControl_MouseClick(object sender, MouseEventArgs e)
        {
            if ((((this._masterPane == null) || (this.MouseClickEvent == null)) || !this.MouseClickEvent(this, e)) && (this._masterPane != null))
            {
                int num;
                GraphPane pane;
                object obj2;
                ...
                if (this.MasterPane.FindNearestPaneObject((PointF)mousePt, g, out pane, out obj2, out num))
                {
                    if ((obj2 is CurveItem) && (num >= 0))
                    {
                        CurveItem curve = (CurveItem)obj2;
                        if (this.PointClickEvent != null)
                        {
                            this.PointClickEvent(this, pane, curve, num);
                        }
                    }
                    else if ((obj2 is GraphPane) && (this.TitleClickEvent != null))
                    {
                        this.TitleClickEvent(this, pane);
                    }
                    g.Dispose();
                }
            }
        ...
        protected void ZedGraphControl_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            if ((((this._masterPane == null) || (this.MouseDoubleClickEvent == null)) || !this.MouseDoubleClickEvent(this, e)) && ((this._masterPane != null) && (this._dragPane == null)))
            {
                int num;
                GraphPane pane;
                object obj2;
                ...
                if (this.MasterPane.FindNearestPaneObject((PointF)mousePt, g, out pane, out obj2, out num))
                {
                    ...
                    {
                        CurveItem curve = (CurveItem)obj2;
                        if (this.PointDoubleClickEvent != null)
                        {
                            this.PointDoubleClickEvent(this, pane, curve, num);
                        }
                    }
                    else if (obj2 is GraphPane)
                    {
                        if (this.TitleDoubleClickEvent != null)
                        {
                            this.TitleDoubleClickEvent(this, pane);
                        }
                    }
                    else if (((obj2 is Legend) && (num >= 0)) && (this.LegendDoubleClickEvent != null))
                    {
                        this.LegendDoubleClickEvent(this, pane, pane.CurveList[num]);
                    }
                    g.Dispose();
                }
                else
                {
                    this.IsShowHorizontalGuideLine = !this.IsShowHorizontalGuideLine;
                    this.IsShowVerticalGuideLine = !this.IsShowVerticalGuideLine;
                    this.IsShowPointValues = !this.IsShowPointValues;
                    Rectangle rectangle = base.RectangleToScreen(new Rectangle(new Point(0, 0), base.Size));
                    //if (this.IsShowVerticalGuideLine)
                    //{
                    ControlPaint.DrawReversibleLine(new Point(Control.MousePosition.X, rectangle.Top), new Point(Control.MousePosition.X, rectangle.Bottom), Color.Black);
                    //}
                    //if (this.IsShowHorizontalGuideLine)
                    //{
                    ControlPaint.DrawReversibleLine(new Point(rectangle.Left, Control.MousePosition.Y), new Point(rectangle.Right, Control.MousePosition.Y), Color.Black);
                    //}
                }
            }
		...
		protected void ZoomScale( Axis axis, double zoomFraction, double centerVal, bool isZoomOnCenter )
		{
			if ( axis != null && zoomFraction > 0.0001 && zoomFraction < 1000.0 )
			{
				...
				*/
				...
				axis.Scale._minLinearized = centerVal - range;
				axis.Scale._maxLinearized = centerVal + range;
				//				}
	...
	#region Edit Point Events
		...
		private void HandleEditDrag( Point mousePt )
		{
			...
			Scale xScale = _dragCurve.GetXAxis( _dragPane ).Scale;
			if ( _isEnableHEdit )
				newPt.X = xScale.DeLinearize( xScale.Linearize( newPt.X ) +
							xScale.Linearize( curX ) - xScale.Linearize( startX ) );

			Scale yScale = _dragCurve.GetYAxis( _dragPane ).Scale;
			if ( _isEnableVEdit )
		...
		private void HandleZoomFinish( object sender, MouseEventArgs e )
		{
					...
					( Math.Abs( mousePtF.Y - _dragStartPt.Y ) > 4 || !_isEnableVZoom ) )
			{
				...
				if ( !zoomLimitExceeded )
				{
					...
					if ( _isEnableHZoom )
					{
						_dragPane.XAxis.Scale._min = Math.Min( x1, x2 );
						_dragPane.XAxis.Scale.MinAuto = false;
						_dragPane.XAxis.Scale._max = Math.Max( x1, x2 );
						_dragPane.XAxis.Scale.MaxAuto = false;

						_dragPane.X2Axis.Scale._min = Math.Min( xx1, xx2 );
						_dragPane.X2Axis.Scale.MinAuto = false;
						_dragPane.X2Axis.Scale._max = Math.Max( xx1, xx2 );
					...
					if ( _isEnableVZoom )
					{
						for ( int i = 0; i < y1.Length; i++ )
						{
							_dragPane.YAxisList[i].Scale._min = Math.Min( y1[i], y2[i] );
							_dragPane.YAxisList[i].Scale._max = Math.Max( y1[i], y2[i] );
							_dragPane.YAxisList[i].Scale.MinAuto = false;
							_dragPane.YAxisList[i].Scale.MaxAuto = false;
						}
						for ( int i = 0; i < yy1.Length; i++ )
						{
							_dragPane.Y2AxisList[i].Scale._min = Math.Min( yy1[i], yy2[i] );
							_dragPane.Y2AxisList[i].Scale._max = Math.Max( yy1[i], yy2[i] );

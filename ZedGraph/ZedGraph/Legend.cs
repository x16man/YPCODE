				...
				foreach ( GraphPane tmpPane in paneList )
				{
					...
					for ( int i = 0; i < count; i++ )
					{
						...
						if ( curve._label._text != "" && curve._label._isVisible )
						{
							...
							{
                                //Wang Junhui modified.
                                tmpFont.Draw(g, pane, curve._label._text + (curve.IsLine?(" : " + curve.CurrentPtValue):""),
										x + 2.5F * _tmpSize, y + _legendItemHeight / 2.0F + 1.0F,
							...
							else
							{
                                ...
                                //Wang Junhui modified.
                                tmpFont.Draw(g, pane, curve._label._text + " : " + curve.CurrentPtValue,
									x + 0.0F * _tmpSize, y + _legendItemHeight / 2.0F + 1.0F,
									AlignH.Left, AlignV.Center, scaleFactor);
							}
			...
			foreach ( GraphPane tmpPane in paneList )
			{
				...
				for ( int i = 0; i < count; i++ )
				{
					...
					if ( curve._label._text != string.Empty && curve._label._isVisible )
					{
                        ...
                        tmpWidth = tmpFont.GetWidth(g, curve._label._text + " : " + curve.CurrentPtValue, scaleFactor);

...
namespace ZedGraph
{
	...
	#region Fields
		...
		/// </summary>
		internal double	_min,_max,_majorStep,_minorStep;
...
//					grid.Draw( g, gridPen, pixVal2, topPix );
					...
					bool isSkipZone = ( ( ( IsSkipFirstLabel && isMaxValueAtMaxPix ) ||
		...
		{
			MajorTic tic = _ownerAxis.MajorTic;
			MajorGrid grid = _ownerAxis.MajorGrid;
...
//				float lastPixVal = -10000;
						...
						float shift, float maxSpace, float scaledTic, float charHeight, float scaleFactor )
		{
			...
			if ( _ownerAxis.MajorTic.IsOutside )
				textTop = scaledTic + charHeight * LabelGap;
			else
				textTop = charHeight * LabelGap;
			...
			if ( Align == AlignP.Center )
				textCenter = textTop + maxSpace / 2.0F;
			else if ( Align == AlignP.Outside )
			...
			if ( _ownerAxis is XAxis || _ownerAxis is X2Axis )
				...
				av = AlignH == AlignH.Left ? AlignV.Top : ( AlignH == AlignH.Right ? AlignV.Bottom : AlignV.Center );
	...
	#region Coordinate Transform Methods
		...
		public float Transform( double x )
		{
			...
			if ( IsReverse == ( _ownerAxis is XAxis || _ownerAxis is X2Axis ) )
		...
		public float LocalTransform( double x )
		{
			...
			if ( IsReverse == ( _ownerAxis is YAxis || _ownerAxis is X2Axis ) )

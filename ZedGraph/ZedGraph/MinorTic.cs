...
namespace ZedGraph
{
	...
	public class MinorTic : ICloneable, ISerializable
	{
		...
		public MinorTic()
		{
			...
			IsCrossOutside = Default.IsCrossOutside;
			IsCrossInside = Default.IsCrossInside;
		}
		...
		public MinorTic( MinorTic rhs )
		{
			...
			this.PenWidth = rhs.PenWidth;

			this.IsOutside = rhs.IsOutside;
			this.IsInside = rhs.IsInside;
			this.IsOpposite = rhs.IsOpposite;
			this.IsCrossOutside = rhs.IsCrossOutside;
			this.IsCrossInside = rhs.IsCrossInside;
		}
		...
		protected MinorTic( SerializationInfo info, StreamingContext context )
		{
			...
			this.PenWidth = info.GetSingle( "penWidth" );
			this.IsOutside = info.GetBoolean( "IsOutside" );
			this.IsInside = info.GetBoolean( "IsInside" );
			this.IsOpposite = info.GetBoolean( "IsOpposite" );
			this.IsCrossOutside = info.GetBoolean( "isCrossOutside" );
			this.IsCrossInside = info.GetBoolean( "isCrossInside" );
		}
		...
		public virtual void GetObjectData( SerializationInfo info, StreamingContext context )
		{
			...
			info.AddValue( "isCrossOutside", IsCrossOutside );
			info.AddValue( "isCrossInside", IsCrossInside );
					...
					float shift, float scaledTic )
		{
            ...
            if (this.IsOutside)
            {
                ...
                //Logger.Info(string.Format("({0},{1}),({2},{3})", pixVal, shift, pixVal, shift + scaledTic));
            }
            ...
            {
                g.DrawLine(pen, pixVal, shift, pixVal, shift - scaledTic);
                //Logger.Info(string.Format("MinorTic({0},{1}),({2},{3})",pixVal, shift,pixVal, shift - scaledTic));
            }

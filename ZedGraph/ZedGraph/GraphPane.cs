...
namespace ZedGraph
{
	...
	#region General Properties
	    ...
	    [Bindable( true ), Browsable( true ), Category( "Display" ), NotifyParentProperty( true ), Description("Determines whether the auto-ranged scale will include all data points" +
	    ...
	    /// </summary>
	    public ZoomStateStack ZoomStack { get; private set; }
	...
	#region Rendering Methods
		...
		public void AxisChange( Graphics g )
		{
			...
			if ( this.CurveList.IsPieOnly )//如果全是饼图。
			{
		...
		public override void Draw( Graphics g )
		{
		    ...
		    //_chart.Fill.Color = Color.Green;//TODO:Comment it as soon.
...
//			minSpaceT = Math.Max( minSpaceT, spaceT );
	...
	#region AddCurve Methods
        ...
        public BarItem AddBar( string label, IPointList points, Color color, Color negativeColor)
        {
            BarItem curve = new BarItem(label, points, color, negativeColor);
            _curveList.Add(curve);
        ...
        public BarItem AddBar( string label, double[] x, double[] y, Color color, Color negativeColor)
        {
            BarItem curve = new BarItem(label, x, y, color);
            _curveList.Add(curve);
	...
	#region General Utility Methods
        ...
        public bool FindNearestPointByX(PointF mousePt,
            out CurveItem nearestCurve, out int iNearest)
        {
            return FindNearestPointByX(mousePt, _curveList,
                out nearestCurve, out iNearest);
        }
        ...
        /// <see cref="Default.NearestTol"/> pixels
        /// of the screen point, false otherwise.</returns>
        public bool FindNearestPointByX(PointF mousePt, CurveList targetCurveList,
            out CurveItem nearestCurve, out int iNearest)
        {
            CurveItem nearestBar = null;
            int iNearestBar = -1;
                                    ...
                                    yVal != PointPair.Missing)
                            {
                                    ...
                                    curve is JapaneseCandleStickItem)
                                {
                                    ...
                                    if (isXBaseAxis)
                                    {
                                        ...
                                        //修改前。-------------
                                        //修改后。-------------
                                        distX = centerVal - xAct;
                                        //distX = (centerVal - xAct) * xPixPerUnit;
                                        if (Math.Abs(distX) >= Math.Abs(minDistBar))
                                            continue;
                                        minDistBar = distX;
                                        iNearestBar = iPt;
                                        nearestBar = curve;
                                            ...
                                            yVal >= yMinAct && yVal <= yMaxAct)
                                {
                                    ...
                                    ////修改前。-------------
                                    //修改后。--------------
                                    if (Math.Abs(distX) >= Math.Abs(minDist))
                                        continue;
                                    minDist = distX;
                                    iNearest = iPt;
                                    nearestCurve = curve;

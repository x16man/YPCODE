...
namespace MZHMM.WebMM
{
	...
	/// </summary>
	public partial class ROSDetails : System.Web.UI.Page
	{
		protected ROSDetailsData oData;
		#region 属性
		/// <summary>
		/// 用途分类。
		/// </summary>
		public string ClassifyName
		{
			...
			set 
			{
				((Label)this.WebPanel1.FindControl("lblClassify")).Text = value;
			}
		}
		/// <summary>
		/// 用途
		...
		public string ReqReason
		{
			...
			set 
			{
				((Label)this.WebPanel1.FindControl("lblReqReason")).Text = value;
			}
		}
		/// <summary>
		/// 部门
		...
		public string AuthorDeptName
		{
			...
			set 
			{
				((Label)this.WebPanel1.FindControl("lblAuthorDeptName")).Text = value;
			}
		}
		/// <summary>
		/// 总金额。
...
//			this.UltraWebGrid1.Bands[0].Columns.FromKey("ReqReason").Width = new Unit("120px");
		...
		{
			oData = new ItemSystem().Get_ROSDetails(this.ClassifyName,this.ReqReason,this.AuthorDeptName,this.StartDate,this.EndDate, this.Flag);
			this.UltraWebGrid1.DataSource = oData.Tables[ROSDetailsData.ROSDetails_Table].DefaultView;
		}

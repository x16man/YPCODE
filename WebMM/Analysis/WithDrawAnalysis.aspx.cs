...
namespace WebMM.Analysis
{
	...
	/// </summary>
	public partial class WithDrawAnalysis : System.Web.UI.Page
	{
		#region 成员变量
		...
		public string Classify
		{
			get 
			{
				if (this.Request["Classify"] != null && this.Request["Classify"] != "") 
				{
		...
		protected void UltraWebGrid1_DataBinding(object sender, System.EventArgs e)
		{
			...
			oHT.Add("@Classify",this.Classify);
			WithDrawDS = new DataSet();
			WithDrawDS = MySQLServer.ExecSPReturnDS("Analysis_GetWithDrawGroup",oHT);
			...
			oHT.Add("@EndDate",this.EndDate);
			oHT.Add("@Classify", this.Classify);
			oData =	new SQLServer().ExecSPReturnDS("Analysis_WithDrawGetGroupByLayOutAndDate",oHT); 
		...
		private void UltraWebGrid1_InitializeRow(object sender, Infragistics.WebUI.UltraWebGrid.RowEventArgs e)
		{
			...
			{
				//e.Row.Cells.FromKey("ItemMoney").TargetURL = "www.sina.com";
			    string strValue = "";
                if (e.Row.Cells.FromKey("Classify").Value != null)
                    strValue = e.Row.Cells.FromKey("Classify").Value.ToString();
				switch (this.LayOut)
				{
					case "1"://"Classify,ReqReason,AuthorDeptName":
						e.Row.Cells.FromKey("Classify").TargetURL = "@[_blank]WithDrawDetail.aspx?"
							+ "StartDate="+this.StartDate.ToShortDateString()
							+ "&EndDate="+this.EndDate.ToShortDateString()
                            + "&ClassifyName=" + Server.UrlEncode(strValue);

						e.Row.Cells.FromKey("ReqReason").TargetURL = "@[_blank]WithDrawDetail.aspx?"
																+ "StartDate="+this.StartDate.ToShortDateString()
																+ "&EndDate="+this.EndDate.ToShortDateString()
                                                                + "&ClassifyName=" + Server.UrlEncode(strValue) 
																+ "&ReqReason="+Server.UrlEncode(e.Row.Cells.FromKey("ReqReason").Value.ToString());
						break;
					case "2"://"Classify,AuthorDeptName,ReqReason":
						e.Row.Cells.FromKey("Classify").TargetURL = "@[_blank]WithDrawDetail.aspx?"
							+ "StartDate="+this.StartDate.ToShortDateString()
							+ "&EndDate="+this.EndDate.ToShortDateString()
                            + "&ClassifyName=" + Server.UrlEncode(strValue);

						e.Row.Cells.FromKey("AuthorDeptName").TargetURL = "@[_blank]WithDrawDetail.aspx?"
							+ "StartDate="+this.StartDate.ToShortDateString()
							+ "&EndDate="+this.EndDate.ToShortDateString()
                            + "&ClassifyName=" + Server.UrlEncode(strValue) 
							+ "&AuthorDeptName="+Server.UrlEncode(e.Row.Cells.FromKey("AuthorDeptName").Value.ToString());
						break;
					case "3"://"ReqReason,Classify,AuthorDeptName":
						...
						e.Row.Cells.FromKey("ReqReason").TargetURL = "@[_blank]WithDrawDetail.aspx?"
							...
							+ "&ReqReason="+Server.UrlEncode(e.Row.Cells.FromKey("ReqReason").Value.ToString());
						break;
					case "4"://"ReqReason,AuthorDeptName,Classify":
						e.Row.Cells.FromKey("ReqReason").TargetURL = "@[_blank]WithDrawDetail.aspx?"
							+ "StartDate="+this.StartDate.ToShortDateString()
							+ "&EndDate="+this.EndDate.ToShortDateString()	
							+ "&ReqReason="+Server.UrlEncode(e.Row.Cells.FromKey("ReqReason").Value.ToString());
						break;
					case "5"://"AuthorDeptName,Classify,ReqReason":
						e.Row.Cells.FromKey("AuthorDeptName").TargetURL = "@[_blank]WithDrawDetail.aspx?"
							+ "StartDate="+this.StartDate.ToShortDateString()
							+ "&EndDate="+this.EndDate.ToShortDateString()	
							+ "&AuthorDeptName="+Server.UrlEncode(e.Row.Cells.FromKey("AuthorDeptName").Value.ToString());
						break;
					case "6"://"AuthorDeptName,ReqReason,Classify":
						e.Row.Cells.FromKey("AuthorDeptName").TargetURL = "@[_blank]WithDrawDetail.aspx?"
							+ "StartDate="+this.StartDate.ToShortDateString()
							+ "&EndDate="+this.EndDate.ToShortDateString()	
							+ "&AuthorDeptName="+Server.UrlEncode(e.Row.Cells.FromKey("AuthorDeptName").Value.ToString());
				...
				e.Row.Cells.FromKey("ItemMoney").TargetURL = "@[_blank]WithDrawDetail.aspx?"
					                                       + "StartDate="+this.StartDate.ToShortDateString()
														   + "&EndDate="+this.EndDate.ToShortDateString()
                                                           + "&ClassifyName=" + Server.UrlEncode(strValue) 
					                                       + "&ReqReason="+Server.UrlEncode(e.Row.Cells.FromKey("ReqReason").Value==null?string.Empty:e.Row.Cells.FromKey("ReqReason").Value.ToString())
														   + "&AuthorDeptName="+Server.UrlEncode(e.Row.Cells.FromKey("AuthorDeptName").Value.ToString());			
			}
		}

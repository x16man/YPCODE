...
Where   RightCode = @OldRightCode";
        ...
        /// 删除权限记录的SQL语句。
        /// </summary>
        private const string SQL_DELETE_RIGHT = @"
Delete From mySystemRoleRights Where RightCode = @RightCode
Delete From mySystemRights Where RightCode = @RightCode";
        /// <summary>
...
Union
Select Count(*) as aa From mySystemRoleRights Where RightCode = @RightCode) as a"; 
        #endregion
        ...
        private static SqlParameter[] GetRightParameters()
        {
            var parms = SqlHelperParameterCache.GetCachedParameterSet(ConnectionString.PubData, SQL_INSERT_RIGHT);
            if (parms == null)
            {
                parms = new[]
                            {
                                ...
                                new SqlParameter("@RightCatCode", SqlDbType.NVarChar, 10), 
                                new SqlParameter("@OldRightCode", SqlDbType.SmallInt), 
                            };

                SqlHelperParameterCache.CacheParameterSet(ConnectionString.PubData, SQL_INSERT_RIGHT, parms);
            }
            return parms;
        }
        /// <summary>
        ...
        public bool Insert(RightInfo rightInfo)
        {
            ...
            try
            {
                SqlHelper.ExecuteNonQuery(ConnectionString.PubData, CommandType.Text, SQL_INSERT_RIGHT, parms);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool Update(RightInfo rightInfo)
        {
            ...
            try
            {
                SqlHelper.ExecuteNonQuery(ConnectionString.PubData, CommandType.Text, SQL_UPDATE_RIGHT, parms);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool Delete(RightInfo rightInfo)
        {
            ...
            try
            {
                SqlHelper.ExecuteNonQuery(ConnectionString.PubData, CommandType.Text, SQL_DELETE_RIGHT, parms);
                return true;
            }
            catch (Exception ex)
            {
        ...
        {
            var parms = GetRightParameters();
            parms[5].Value = rightCatCode;
            var objs = new ListBase<RightInfo>();
            var dr = SqlHelper.ExecuteReader(ConnectionString.PubData, CommandType.Text, SQL_SELECT_ALL_BY_RIGHTCATCODE,
                                             parms);
            while (dr.Read())
            {
                objs.Add(ConvertToRightInfo(dr));
        ...
        public IList<RightInfo> GetAllAvalibleByRightCatCode(string rightCatCode)
        {
            ...
            parms[5].Value = rightCatCode;
            var objs = new ListBase<RightInfo>();
            var dr = SqlHelper.ExecuteReader(ConnectionString.PubData, CommandType.Text, SQL_SELECT_ALLAVALIBLE_BY_RIGHTCATCODE,
                                             parms);
            while (dr.Read())
            {
                objs.Add(ConvertToRightInfo(dr));
        ...
        public IList<RightInfo> GetAllOtherByProductCode(short productCode)
        {
            ...
            parms[4].Value = productCode;
            var objs = new ListBase<RightInfo>();
            var dr = SqlHelper.ExecuteReader(ConnectionString.PubData, CommandType.Text, SQL_SELECT_ALL_OTHER_BY_PRODUCTCODE,
                                             parms);
            while (dr.Read())
            {
                objs.Add(ConvertToRightInfo(dr));
        ...
        public IList<RightInfo> GetAllAvalibleOtherByProductCode(short productCode)
        {
            ...
            parms[4].Value = productCode;
            var objs = new ListBase<RightInfo>();
            var dr = SqlHelper.ExecuteReader(ConnectionString.PubData, CommandType.Text, SQL_SELECT_ALLAVALIBLE_OTHER_BY_PRODUCTCODE,
                                             parms);
            while (dr.Read())
            {
                objs.Add(ConvertToRightInfo(dr));
        ...
        public RightInfo GetByCode(short rightCode)
        {
            ...
            parms[0].Value = rightCode;
            RightInfo obj = null;
            var dr = SqlHelper.ExecuteReader(ConnectionString.PubData, CommandType.Text, SQL_SELECT_BY_RIGHTCODE,
                                             parms);
            while (dr.Read())
            {
        ...
        {
            var parms = new[] {new SqlParameter("@RightCode", SqlDbType.SmallInt) {Value = rightCode}};
            var obj = SqlHelper.ExecuteScalar(ConnectionString.PubData, CommandType.Text, SQL_ISUSING_BY_CODE, parms);
            return (int) obj == 0 ? false : true;
        }

...
namespace Shmzh.Components.SystemComponent.SQLServerDAL
{
    ...
    public class EmpState :IEmpState
    {
        ...
        /// </summary>
        /// <returns></returns>
        private static SqlParameter[] GetEmpStateParameters()
        {
            var parms = SqlHelperParameterCache.GetCachedParameterSet(ConnectionString.PubData, SQL_INSERT_EMPSTATE);
            if (parms == null)
            {
                parms = new[]
                            {
                                new SqlParameter("@Code", SqlDbType.NChar,1),
                                new SqlParameter("@Description", SqlDbType.NVarChar,20), 
                                new SqlParameter("@IsValid", SqlDbType.NChar,10),
                            };

                SqlHelperParameterCache.CacheParameterSet(ConnectionString.PubData, SQL_INSERT_EMPSTATE, parms);
            }
            return parms;
        }
        /// <summary>
        ...
        public bool Insert(EmpStateInfo empStateInfo)
        {
            ...
            try
            {
                SqlHelper.ExecuteNonQuery(ConnectionString.PubData, CommandType.Text, SQL_INSERT_EMPSTATE, parms);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool Update(EmpStateInfo empStateInfo)
        {
            ...
            try
            {
                SqlHelper.ExecuteNonQuery(ConnectionString.PubData, CommandType.Text, SQL_UPDATE_EMPSTATE, parms);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool Delete(EmpStateInfo empStateInfo)
        {
            ...
            try
            {
                SqlHelper.ExecuteNonQuery(ConnectionString.PubData, CommandType.Text, SQL_DELETE_EMPSTATE, parms);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool IsExistCode(string code)
        {
            var parms = GetEmpStateParameters();
            parms[0].Value = code;
            var obj = SqlHelper.ExecuteScalar(ConnectionString.PubData, CommandType.Text, SQL_SELECT_COUNT_BY_CODE,
                                              parms);
        ...
        public bool IsExistDescription(string description)
        {
            ...
            var obj = SqlHelper.ExecuteScalar(ConnectionString.PubData, CommandType.Text, SQL_SELECT_COUNT_BY_DESCRIPTION,
                                              parms);
            return (int)obj == 0 ? false : true;
        }

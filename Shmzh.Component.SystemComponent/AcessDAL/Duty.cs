...
namespace Shmzh.Components.SystemComponent.AccessDAL
{
    ...
    public class Duty :IDuty
    {
        ...
        /// </summary>
        /// <returns></returns>
        private static OleDbParameter[] GetDutyParameters()
        {
            var parms = AccessHelperParameterCache.GetCachedParameterSet(ConnectionString.PubData, SQL_INSERT_DUTY);
            if (parms == null)
            {
                parms = new[]
                            {
                                new OleDbParameter("@DutyCo", OleDbType.VarChar,20),
                                new OleDbParameter("@DutyCode", OleDbType.VarChar,20), 
                                new OleDbParameter("@ParentDutyCode", OleDbType.VarChar,20),
                                new OleDbParameter("@DutyCnName", OleDbType.VarChar,50),
                                new OleDbParameter("@DutyEnName", OleDbType.VarChar,50),
                                new OleDbParameter("@IsValid", OleDbType.Char,1), 
                                new OleDbParameter("@DutyLevel", OleDbType.SmallInt),
                                new OleDbParameter("@Remark", OleDbType.VarChar,50), 
                            };

                AccessHelperParameterCache.CacheParameterSet(ConnectionString.PubData, SQL_INSERT_DUTY, parms);
            }
            return parms;
        }
        /// <summary>
        ...
        public bool Insert(DutyInfo dutyInfo)
        {
            ...
            try
            {
                AccessHelper.ExecuteNonQuery(ConnectionString.PubData, SQL_INSERT_DUTY, parms);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool Update(DutyInfo dutyInfo)
        {
            ...
            try
            {
                AccessHelper.ExecuteNonQuery(ConnectionString.PubData, SQL_UPDATE_DUTY, parms);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool Delete(DutyInfo dutyInfo)
        {
            ...
            try
            {
                AccessHelper.ExecuteNonQuery(ConnectionString.PubData, SQL_DELETE_DUTY, parms);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool IsExistDutyCode(string companyCode, string dutyCode)
        {
            ...
            parms[0].Value = companyCode;
            parms[1].Value = dutyCode;
            var obj = AccessHelper.ExecuteScalar(ConnectionString.PubData, SQL_SELECT_COUNT_BY_DUTYCODE,
                                              parms);
        ...
        public bool IsExistDutyName(string companyCode, string dutyName)
        {
            ...
            var obj = AccessHelper.ExecuteScalar(ConnectionString.PubData, SQL_SELECT_COUNT_BY_DUTYNAME,
                                              parms);
            return (int)obj == 0 ? false : true;
        }
        ...
        public DutyInfo GetByCompanyCodeAndDutyCode(string companyCode, string dutyCode)
        {
            ...
            parms[1].Value = dutyCode;
            DutyInfo obj = null;
            var dr = AccessHelper.ExecuteReader(ConnectionString.PubData,  SQL_SELECT_BY_COMPANY_DUTYCODE,
                                             parms);
            while (dr.Read())
            {

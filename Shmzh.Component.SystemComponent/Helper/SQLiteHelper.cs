...
namespace Shmzh.Components.SystemComponent
{
	...
	public class SQLiteHelper
	{
		...
		{
			var parameter = new SQLiteParameter {DbType = parameterType, ParameterName = parameterName, Value = parameterValue};
		    return parameter;
		}
		...
		public static DataSet ExecuteDataSet(string connectionString, string commandText, object[] paramList)
		{
			...
			if(paramList!=null)
			{
				var parms= DeriveParameters(commandText,paramList);
		...
		public static DataSet ExecuteDataset(SQLiteTransaction transaction,  string commandText,   object[] commandParameters)
		{
			...
			IDbCommand cmd = transaction.Connection.CreateCommand();
			cmd.CommandText =commandText;
			var coll= DeriveParameters(cmd.CommandText,commandParameters);
        ...
        /// <returns></returns>
        public static IDataReader ExecuteReader(SQLiteCommand cmd, string commandText, params SQLiteParameter[] commandParameters)
        {
            if (cmd.Connection == null)
                throw new ArgumentException("Command must have live connection attached.", "cmd");
            cmd.CommandText = commandText;
		...
		{					
			...
			IDbCommand cmd = transaction.Connection.CreateCommand();
			cmd.CommandText =commandText;
			var coll= DeriveParameters(cmd.CommandText,paramList);
			foreach(var parm in coll)
				cmd.Parameters.Add(parm);
			if(transaction.Connection.State ==ConnectionState.Closed)
				transaction.Connection.Open();
			var result = cmd.ExecuteNonQuery();
		...
		public static List<SQLiteParameter> DeriveParameters(string commandText,object[] paramList)
		{
			if(paramList==null)return null;

		    var coll = new List<SQLiteParameter>();
		...
		protected void AssignParameterValues(IDataParameter[] commandParameters, object[] parameterValues)
		{
			...
			for (int i = 0, j = commandParameters.Length, k = 0; i < j; i++)
			{
				if (commandParameters[i].Direction != ParameterDirection.ReturnValue)
				{
					...
					else commandParameters[i].Value = parameterValues[k] ?? DBNull.Value;
					k++;
				}
			}
		}
